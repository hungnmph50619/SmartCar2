    ...
    public async Task<DashboardDto> GetAsync(CancellationToken cancellationToken = default)
    {
        ...
        return new DashboardDto
        {
            TotalVehicles = await _dbContext.Vehicles.CountAsync(
                vehicle => vehicle.Status != VehicleStatus.Inactive,
                cancellationToken),
            AvailableVehicles = await _dbContext.Vehicles.CountAsync(
                vehicle => vehicle.Status == VehicleStatus.Available,
                cancellationToken),
            RentedVehicles = await _dbContext.Vehicles.CountAsync(
                vehicle => vehicle.Status == VehicleStatus.Rented,
                cancellationToken),
            InspectionVehicles = await _dbContext.Vehicles.CountAsync(
                vehicle => vehicle.Status == VehicleStatus.Inspection,
                cancellationToken),
            MaintenanceVehicles = await _dbContext.Vehicles.CountAsync(
            ...
            TodayPickups = await _dbContext.Bookings.CountAsync(
                booking =>
                    ...
                    booking.PickupDate < tomorrow &&
                    (booking.Status == BookingStatus.Paid ||
                     booking.Status == BookingStatus.ReadyForPickup),
                cancellationToken),
            TodayReturns = await _dbContext.Bookings.CountAsync(
                booking =>
                    booking.ReturnDate >= today &&
                    booking.ReturnDate < tomorrow &&
