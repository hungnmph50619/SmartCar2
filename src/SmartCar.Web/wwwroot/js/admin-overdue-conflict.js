(() => {
    ...
    const render = data => {
        ...
        if (data.state === 'affected') {
            const contractValue = Number(data.affectedContractAmount || 0);
            const deposit = Number(data.availableDeposit || 0);
            const deduction = Number(data.depositDeduction || 0);
            const remainingDeposit = Math.max(0, deposit - deduction);
            ...
            card.innerHTML = `
                ...
                <div class="row g-3 mt-2">
                    <div class="col-md-4"><div class="small text-muted">Hạn trả #${bookingId}</div><strong>${dateTime(data.returnDate)}</strong></div>
                    <div class="col-md-4"><div class="small text-muted">Đơn bị ảnh hưởng</div><strong>#${data.affectedBookingId}</strong><div class="small text-muted">Nhận ${dateTime(data.affectedPickupDate)}</div></div>
                    <div class="col-md-4"><div class="small text-muted">Giá trị hợp đồng #${data.affectedBookingId}</div><strong class="text-danger fs-5">${money(contractValue)}</strong><div class="small text-muted">Tiền thuê + phí dịch vụ của đơn; không bao gồm tiền cọc.</div></div>
                </div>
                <div class="alert alert-light border mt-3 mb-0 small">
                    <strong>Căn cứ:</strong> theo điều khoản đã thỏa thuận, khi khách #${bookingId} bị từ chối gia hạn nhưng vẫn cố tình giữ xe và thực tế làm đơn #${data.affectedBookingId} không nhận được xe, <strong>khoản xử lý vi phạm bằng giá trị hợp đồng của đơn bị ảnh hưởng: ${money(contractValue)}</strong>.
                </div>
                <div class="rounded border bg-light p-3 mt-3">
                    <div class="fw-bold mb-2">Dự kiến xử lý tiền của đơn #${bookingId}</div>
                    <div class="d-flex justify-content-between py-1"><span>Cọc đang giữ</span><strong>${money(deposit)}</strong></div>
                    <div class="d-flex justify-content-between py-1"><span>Khoản xử lý vi phạm</span><strong class="text-danger">${money(contractValue)}</strong></div>
                    <div class="d-flex justify-content-between py-1"><span>Tạm khấu trừ từ cọc</span><strong>${money(deduction)}</strong></div>
                    <div class="d-flex justify-content-between py-1"><span>Cọc còn lại chờ quyết toán</span><strong>${money(remainingDeposit)}</strong></div>
                    <div class="d-flex justify-content-between border-top pt-2 mt-2"><span>Còn phải thu thêm từ khách #${bookingId}</span><strong class="${outstanding > 0 ? 'text-danger' : 'text-success'}">${money(outstanding)}</strong></div>
                </div>
                <div class="alert alert-danger mt-3 mb-3"><strong>Chưa tự động trừ cọc.</strong> Xe chưa trả nên cọc vẫn đang được SmartCar giữ. Chỉ sau khi Admin xác nhận vi phạm, hệ thống mới ghi nhận khoản khấu trừ; phần cọc còn lại tiếp tục chờ trả xe, kiểm tra xe và quyết toán cuối chuyến.</div>
                <form action="/AdminOverdueViolations/Process" method="post" onsubmit="return confirm('Xác nhận đơn #${bookingId} đã bị từ chối gia hạn nhưng vẫn giữ xe quá hạn, làm ảnh hưởng đơn #${data.affectedBookingId}? Khoản xử lý vi phạm là ${money(contractValue)}.');">
                    ...
                    <div class="d-flex gap-2 flex-wrap"><a class="btn btn-outline-secondary" href="/AdminBookings/Details/${data.affectedBookingId}">Xem đơn #${data.affectedBookingId}</a><button class="btn btn-danger" type="submit">Xác nhận & xử lý vi phạm</button></div>
                </form>`;
