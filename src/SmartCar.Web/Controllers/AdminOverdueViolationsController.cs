...
public sealed class AdminOverdueViolationsController : Controller
{
    ...
    private static decimal CalculateAvailableDeposit(IEnumerable<Payment> payments)
    {
        ...
        var refunded = payments.Where(x => x.Type == PaymentType.Refund && x.Method == PaymentMethods.DepositRefund && x.Status is PaymentStatus.AwaitingRefund or PaymentStatus.Refunded).Sum(x => x.Amount);
